(ns oracle-xstore.main
  "Kotodama WASM entrypoint for the oracle_xstore clean-room actor (L4) — Clojure port.
  ...
  facts (`oracle_xstore.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "oracle_xstore")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "oraclexs_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "oraclexs_cus"
    :fields [:email :firstName :lastName :phone]    :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "oraclexs_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"oracle_xstore.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "oracle_xstore-compat" :tier tier :entities entities} 200])
...
(defn start! [] :oracle_xstore-compat/ready)
