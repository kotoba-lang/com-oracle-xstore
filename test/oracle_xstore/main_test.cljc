(ns oracle-xstore.main-test
  "Contract + behavioral test for the oracle_xstore-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "oracle_xstore-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'oracle-xstore.main-test)]
